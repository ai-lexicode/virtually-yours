...
export default function NewsletterStats() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/admin/newsletter/history").then((r) => r.json()).catch(() => ({ items: [] })),
    ])
      ...
      {!stats ? (
        ...
        <>
          ...
          {newsletters.length > 0 && (
            <div className="rounded-xl border border-card-border bg-[#1a1a1a] p-5">
              <h3 className="text-white font-semibold mb-4">Per nieuwsbrief</h3>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-card-border text-muted text-left">
                      ...
                      <th className="pb-2 pr-4">Ontvangers</th>
                      <th className="pb-2"></th>
                    </tr>
                  </thead>
                  <tbody>
                    {newsletters.map((nl) => (
                      <tr key={nl.id} className="border-b border-card-border/30 last:border-0">
                        <td className="py-2 pr-4 text-white">{nl.subject}</td>
                        <td className="py-2 pr-4 text-muted">{nl.sentAt ? new Date(nl.sentAt).toLocaleDateString("nl-NL") : "\u2014"}</td>
                        <td className="py-2 pr-4 text-muted">{nl.recipientCount}</td>
                        <td className="py-2">
                          <button onClick={() => setSelectedNewsletter(nl.id)} className="text-primary hover:text-primary/80 p-1">
                            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" />
                            </svg>
                          </button>
